...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        load_model_folder = f"/root/autodl-fs/KDD2026/saved_models/{args.model_name}_{args.exp}/{args.dataset_name}/{args.load_model_name}"
        ...
        early_stopping.load_checkpoint_4test(model, map_location='cpu', epochs='24')

        model = convert_to_gpu(model, device=args.device)
        # put the node raw messages of memory-based models on device
        if args.model_name in ['JODIE', 'DyRep', 'TGN', 'Fluxion']:
        ...
        if args.model_name not in ['JODIE', 'DyRep', 'TGN', 'Fluxion']:
        ...
        logger.info(f'save popularity prediction results at {save_result_path}')
